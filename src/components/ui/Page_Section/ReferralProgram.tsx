...
const ReferralStep = ({
  ...
  return (
    <motion.div
      ...
      className={`flex items-center ${isMobile ? '' : 'my-12'} ${
        alignment === 'left' ? 'md:px-12 pr-4 ' : 'md:pl-12 pl-4'
      } max-w-full`}
    >
      <div
        className={`${isMobile && !title ? 'hidden' : ''} ${
          isMobile
            ? 'backdrop-blur-sm p-6 mt-3 rounded-xl shadow-lg border'
            : ''
